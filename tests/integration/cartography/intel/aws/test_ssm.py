...
def _ensure_load_instances(neo4j_session):
    """
    Note: Callers of this MUST patch over cartography.intel.aws.ec2.instances.get_ec2_instances.
    """
...
def test_load_instance_patches(mock_get_instances, neo4j_session):
    ...
    nodes = neo4j_session.run(
        """
        MATCH (:EC2Instance{instanceid: "i-01"})-[:HAS_PATCH]->(n:SSMInstancePatch)
        RETURN n.id
        """,
    )
    ...
    nodes = neo4j_session.run(
        """
        MATCH (:EC2Instance{instanceid: "i-02"})-[:HAS_PATCH]->(n:SSMInstancePatch)
        RETURN n.id
        """,
    )
    actual_nodes = {n["n.id"] for n in nodes}
