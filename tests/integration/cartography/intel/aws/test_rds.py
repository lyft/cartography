...
def test_load_rds_instances_basic(neo4j_session):
    ...
    query = """MATCH(rds:RDSInstance) RETURN rds.id, rds.storage_encrypted"""
    nodes = neo4j_session.run(query)

    actual_nodes = {(n['rds.id'], n['rds.storage_encrypted']) for n in nodes}
    expected_nodes = {('arn:aws:rds:us-east-1:some-arn:db:some-prod-db-iad-0', True)}
