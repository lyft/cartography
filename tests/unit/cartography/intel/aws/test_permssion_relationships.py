...
from cartography.intel.aws import resource_permissions
...
def test_admin_statments():
    assert True == resource_permissions.evaluate_policy_for_permission(
        ADMIN_STATEMENTS, ["S3:GetObject"], "arn:aws:s3:::testbucket")


def test_not_action_statement():
    assert False == resource_permissions.evaluate_policy_for_permission(
