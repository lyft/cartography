...
from cartography.intel.github.util import fetch_all_deps_and_vulns
...
GITHUB_ORG_REPOS_DEPS_PAGINATED_GRAPHQL = """
    query($login: String!, $repo_name: String!, $cursor: String) {
    organization(login: $login)
    {
          url
          login
...
GITHUB_ORG_REPOS_VULNS_PAGINATED_GRAPHQL = """
    query($login: String!, $repo_name: String!, $cursor: String) {
    organization(login: $login)
    {
          url
...
def get(token: str, api_url: str, organization: str) -> List[Dict]:
    ...
    repos_result = {}
    repos, org_info = fetch_all_nodes(token, api_url, organization, GITHUB_ORG_REPOS_PAGINATED_GRAPHQL, 'repositories', 'nodes')
    for repo in repos:
        repo_name = repo['name']
        repos_result[repo_name] = repo
        repos_result[repo_name]['owner'] = org_info
        deps_raw = fetch_all_deps_and_vulns(
            token, api_url, organization,
            GITHUB_ORG_REPOS_DEPS_PAGINATED_GRAPHQL,
            'repository', 'dependencyGraphManifests',
            provided_vars={"repo_name": repo_name}
        )
        all_repo_deps = []
        for dep_manifests in deps_raw['nodes']:
            all_repo_deps.extend(dep_manifests['dependencies']['nodes'])
        repos_result[repo_name]['deps'] = all_repo_deps
        vulns_raw = fetch_all_deps_and_vulns(
            token, api_url, organization,
            GITHUB_ORG_REPOS_VULNS_PAGINATED_GRAPHQL,
            'repository', 'vulnerabilityAlerts',
            provided_vars={"repo_name": repo['name']}
        )
        repos_result[repo_name]['vulns'] = vulns_raw['nodes']
    return repos
...
def transform(repos_json: List[Dict]) -> Dict:
    ...
    transformed_repo_deps: List[Dict] = []
    transformed_repo_vulns: List[Dict] = []
    for repo_object in repos_json:
        ...
        _transform_deps(repo_object['deps'], repo_object['url'], transformed_repo_deps)
        _transform_vulns(repo_object['vuln'], repo_object['url'], transformed_repo_vulns)
    results = {
        ...
        'repo_vulns': transformed_repo_vulns,
        'repo_deps': transformed_repo_deps
    }
    return results
...
def _transform_deps(input_dep_object: Dict, repo_url: str, out_repo_list: List[Dict]) -> None:
    """
    Performs data transformations for the dependencies for a GitHub repo, if available.
    :param input_dep_object: A repo dependencies node from GitHub; see https://docs.github.com/en/graphql/reference/objects#dependencygraphmanifestconnection for data shape.
    :param repo_url: str: The URL of the GitHub repo.
    :param out_deps_list: Out-param to append transformed results to.
    :return: Nothing
    """
    out_deps_list.append({
        'id': f"{repo_url}::{input_dep_object['packageName']}",
        'name': input_dep_object['packageName'],
        'requirements': input_dep_object['packarequirementsgeName'],
        'packageManager': input_dep_object['packageManager'],
        'packageLabel': input_dep_object['packageLabel'],
        'hasDependencies': input_dep_object['hasDependencies']
    })

def _transform_vulns(input_vulns_object: Dict, repo_url: str, out_vulns_list: List[Dict]) -> None:
    """
    Performs data transformations for the vulns for a GitHub repo, if available.
    :param input_dep_object: A repo dependencies node from GitHub; see https://docs.github.com/en/graphql/reference/objects#repositoryvulnerabilityalertconnection for data shape.
    :param repo_url: str: The URL of the GitHub repo.
    :param out_vulns_list: Out-param to append transformed results to.
    :return: Nothing
    """
    out_vulns_list.append({
        'id': input_vulns_object[''],
        'cvss_score': input_vulns_object['securityAdvisory']['cvss']['score'],
        'description': input_vulns_object['description'],
        'package': input_vulns_object['securityVulnerability']['package']['name'],
        'vulnerableManifestFilename': input_vulns_object['vulnerableManifestFilename'],
        'vulnerableManifestPath': input_vulns_object['vulnerableManifestPath'],
        'vulnerableRequirements': input_vulns_object['vulnerableRequirements']
    })
...
if __name__ == "__main__":
    import json
    gh_creds = json.load(open("github.creds"))
    github_api_key = gh_creds['personal_access']
