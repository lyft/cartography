...
class AppContext:
    def __init__(
        ...
        sns_request_topic=None,
        sns_response_topic=None,
        ...
        self.sns_request_topic = sns_request_topic
        self.sns_response_topic = sns_response_topic
    ...
    def parse(self, config):
        ...
        if config.get('audit'):
            cfg = config['audit']
            self.sns_request_topic = self.get_sns_topic(self.get_value(cfg, ['sns', 'reqTopic']))
            self.sns_response_topic = self.get_sns_topic(self.get_value(cfg, ['sns', 'resTopic']))
            self.aws_iam_compliance_request_topic = self.get_sns_topic(self.get_value(cfg, ['iamcompliance', 'reqTopic']), self.sns_request_topic)
            self.aws_iam_compliance_response_topic = self.get_sns_topic(self.get_value(cfg, ['iamcompliance', 'resTopic']), self.sns_response_topic)
            ...
            self.aws_sg_audit_request_topic = self.get_sns_topic(self.get_value(cfg, ['sgaudit', 'reqTopic']), self.sns_request_topic)
            self.aws_sg_audit_response_topic = self.get_sns_topic(self.get_value(cfg, ['sgaudit', 'resTopic']), self.sns_response_topic)
            self.aws_s3_public_check_request_topic = self.get_sns_topic(self.get_value(cfg, ['s3publiccheck', 'reqTopic']), self.sns_request_topic)
            self.aws_s3_public_check_response_topic = self.get_sns_topic(self.get_value(cfg, ['s3publiccheck', 'resTopic']), self.sns_response_topic)
            self.aws_ec2_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['ec2monitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_ec2_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['ec2monitoring', 'resTopic']), self.sns_response_topic)
            self.aws_ec2_monitoring_outdated_instance_types = self.get_value(cfg, ['ec2monitoring', 'outdatedInstanceTypes'])
            self.aws_rds_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['rdsmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_rds_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['rdsmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_lambda_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['lambdamonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_lambda_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['lambdamonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_es_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['esmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_es_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['esmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_kms_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['kmsmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_kms_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['kmsmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_elb_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['elbmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_elb_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['elbmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_ses_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['sesmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_ses_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['sesmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_dynamodb_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['dynamodbmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_dynamodb_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['dynamodbmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_sns_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['snsmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_sns_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['snsmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_kubernetes_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['kubernetesmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_kubernetes_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['kubernetesmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_kubernetes_monitoring_latest_versions = self.get_value(cfg, ['kubernetesmonitoring', 'latestVersions'])
            self.aws_apigateway_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['apigatewaymonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_apigateway_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['apigatewaymonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_cloudfront_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['cloudfrontmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_cloudfront_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['cloudfrontmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_cloudtrail_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['cloudtrailmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_cloudtrail_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['cloudtrailmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_redshift_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['redshiftmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_redshift_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['redshiftmonitoring', 'resTopic']), self.sns_response_topic)
            ...
            self.aws_cloudformation_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['cloudformationmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_cloudformation_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['cloudformationmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_sqs_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['sqsmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_sqs_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['sqsmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_cloudwatch_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['cloudwatchmonitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_cloudwatch_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['cloudwatchmonitoring', 'resTopic']), self.sns_response_topic)
            self.aws_route53_monitoring_request_topic = self.get_sns_topic(self.get_value(cfg, ['route53monitoring', 'reqTopic']), self.sns_request_topic)
            self.aws_route53_monitoring_response_topic = self.get_sns_topic(self.get_value(cfg, ['route53monitoring', 'resTopic']), self.sns_response_topic)
        ...
        if config.get('inventory'):
            ...
            self.aws_inventory_sync_request_topic = self.get_sns_topic(self.get_value(cfg, ['awsinventorysync', 'reqTopic']), self.sns_request_topic)
            self.aws_inventory_sync_response_topic = self.get_sns_topic(self.get_value(cfg, ['awsinventorysync', 'resTopic']), self.sns_response_topic)
            self.aws_iam_deepdive_request_topic = self.get_sns_topic(self.get_value(cfg, ['awsiamdeepdive', 'reqTopic']), self.sns_request_topic)
            self.aws_iam_deepdive_response_topic = self.get_sns_topic(self.get_value(cfg, ['awsiamdeepdive', 'resTopic']), self.sns_response_topic)
            self.aws_inventory_views_request_topic = self.get_sns_topic(self.get_value(cfg, ['awsinventoryviews', 'reqTopic']), self.sns_request_topic)
            self.aws_inventory_views_response_topic = self.get_sns_topic(self.get_value(cfg, ['awsinventoryviews', 'resTopic']), self.sns_response_topic)

        if config.get('monitoring'):
            cfg = config['monitoring']
            self.aws_remediation_request_topic = self.get_sns_topic(self.get_value(cfg, ['awsremediation', 'reqTopic']), self.sns_request_topic)
            self.aws_remediation_response_topic = self.get_sns_topic(self.get_value(cfg, ['awsremediation', 'resTopic']), self.sns_response_topic)
            ...
            self.aws_cost_anomaly_request_topic = self.get_sns_topic(self.get_value(cfg, ['awscostanomaly', 'reqTopic']), self.sns_request_topic)
            self.aws_cost_anomaly_response_topic = self.get_sns_topic(self.get_value(cfg, ['awscostanomaly', 'resTopic']), self.sns_response_topic)
            ...
            self.aws_cost_saving_request_topic = self.get_sns_topic(self.get_value(cfg, ['awscostsaving', 'reqTopic']), self.sns_request_topic)
            self.aws_cost_saving_response_topic = self.get_sns_topic(self.get_value(cfg, ['awscostsaving', 'resTopic']), self.sns_response_topic)
