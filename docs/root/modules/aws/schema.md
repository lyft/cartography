...
|date_created|The date when the application was created.|
|configuration_templates|The names of the configuration templates associated with this application.|
|resource_lifecycle_config|The lifecycle settings for the application.|
|versions|The names of the versions for this application.|
### Relationships

- ElasticBeanStalk Application is a resource under the AWS Account.

        ```
        (AWSAccount)-[RESOURCE]->(ElasticBeanStalkApplication)
        ```

- ElasticBeanStalk Application HAVE ElasticBeanStalk Environments.

        ```
        (ElasticBeanStalkApplication)-[HAS_ENVIRONMENT]->(ElasticBeanStalkEnvironment)
        ```

- ElasticBeanStalk Application HAVE ElasticBeanStalk Versions.

        ```
        (ElasticBeanStalkApplication)-[HAS_VERSION]->(ElasticBeanStalkVersion)
        ```

### AWS ElasticBeanStalk Version
...
| arn | The ARN of the elasticbeanstalk version |
| date_created | The creation date of the application version. |
| status | The processing status of the application version. Reflects the state of the application version during its creation. Many of the values are only applicable if you specified True for the Process parameter of the CreateApplicationVersion action.  |
| source_build_information | If the version's source code was retrieved from AWS CodeCommit, the location of the source code for the application version. |
| source_bundle | The storage location of the application version's source bundle in Amazon S3. |
### Relationships

- ElasticBeanStalk Versions is a resource under the ElasticBeanStalk Application.

        ```
        (ElasticBeanStalkApplication)-[HAS_VERSION]->(ElasticBeanStalkVersion)
        ```

- ElasticBeanStalk Versions is a running in the ElasticBeanStalk Environment.

        ```
        (ElasticBeanStalkEnvironment)-[IS_RUNNING_VERSION]->(ElasticBeanStalkVersion)
        ```

- ElasticBeanStalk Versions has a source S3Bucket.

        ```
        (ElasticBeanStalkVersion)-[HAS_SOURCE]->(S3Bucket)
        ```
...
|template_name|The name of the configuration template used to originally launch this environment.|
...
|date_created|The creation date for this environment.|
...
|operations_role|The Amazon Resource Name (ARN) of the environment's operations role.|
...
|environment_links|A list of links to other environments in the same group.|
|environment_resources|The detailed description of the AWS resources used by this environment.|

### Relationships

- ElasticBeanStalk Environment is a resource under the ElasticBeanStalk Application.

        ```
        (ElasticBeanStalkApplication)-[HAS_ENVIRONMENT]->(ElasticBeanStalkEnvironment)
        ```

- ElasticBeanStalk Environment is a running a ElasticBeanStalk Version.

        ```
        (ElasticBeanStalkEnvironment)-[IS_RUNNING_VERSION]->(ElasticBeanStalkVersion)
        ```

- ElasticBeanStalk Environment has an AutoScalingGroup as an environment resource.

        ```
        (ElasticBeanStalkEnvironment)-[HAS_ENVIRONMENT_RESOURCE]->(AutoScalingGroup)
        ```

- ElasticBeanStalk Environment has an LaunchConfiguration as an environment resource.

        ```
        (ElasticBeanStalkEnvironment)-[HAS_ENVIRONMENT_RESOURCE]->(LaunchConfiguration)
        ```

- ElasticBeanStalk Environment has an EC2Instance as an environment resource.

        ```
        (ElasticBeanStalkEnvironment)-[HAS_ENVIRONMENT_RESOURCE]->(EC2Instance)
        ```

- ElasticBeanStalk Environment has an LaunchTemplate as an environment resource.

        ```
        (ElasticBeanStalkEnvironment)-[HAS_ENVIRONMENT_RESOURCE]->(LaunchTemplate)
        ```

- ElasticBeanStalk Environment has an LoadBalancer as an environment resource.

        ```
        (ElasticBeanStalkEnvironment)-[HAS_ENVIRONMENT_RESOURCE]->(LoadBalancer)
        ```
