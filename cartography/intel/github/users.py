...
OUTSIDE_COLLABORATORS_PAGINATED_GRAPHQL = """
    query($cursor: String) {
    enterprise(slug: "lyft")
        {
            ownerInfo{
                outsideCollaborators(first:100, after: $cursor){
                    edges {
                        node {
                            url
                            login
...
OUTSIDE_COLLABORATORS_PAGINATED_GRAPHQL_OLD = """
    query($cursor: String) {
    enterprise(slug: "lyft")
        {
            ownerInfo{
...
@timeit
def get_outside_collaborators(token, api_url, organization):
    """
    Retrieve a list of users from the given GitHub organization as described in
    https://docs.github.com/en/graphql/reference/objects#enterpriseoutsidecollaboratorconnection.
    :param token: The Github API token as string.
    :param api_url: The Github v4 API endpoint as string.
    :param organization: The name of the target Github organization as string.
    :return:
    """
    users, org = fetch_all(
        token, api_url, organization, OUTSIDE_COLLABORATORS_PAGINATED_GRAPHQL, 'outsideCollaborators', 'edges',
    )
    return users, org


@timeit
def load_outside_collaborators(users):
    query = """
    UNWIND {UserData} as user
