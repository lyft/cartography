...
def load_ec2_instance_network_interfaces(
    neo4j_session: neo4j.Session, instance_data: Dict[str, Any], aws_update_tag: int,
) -> None:
    ingest_interfaces = """
...
@timeit
def cleanup_ec2_instances(neo4j_session: neo4j.Session, graph_job_parameters: AwsGraphJobParameters) -> None:
    run_cleanup_job('aws_import_ec2_instances_cleanup.json', neo4j_session, graph_job_parameters)


@timeit
def sync_ec2_instances(neo4j_session: neo4j.Session, aws_stage_ctx: AwsStageContext) -> None:
    for region in aws_stage_ctx.current_aws_account_regions:
        logger.info(
            "Syncing EC2 instances for region '%s' in account '%s'.", region, aws_stage_ctx.current_aws_account_id,
        )
        data = get_ec2_instances(aws_stage_ctx.boto3_session, region)
        load_ec2_instances(
            neo4j_session,
            data,
            region,
    ...
    cleanup_ec2_instances(neo4j_session, aws_stage_ctx.graph_job_parameters)
