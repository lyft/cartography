...
def load_network_interface_instance_to_subnet_relations(neo4j_session: neo4j.Session, aws_account_id: str, update_tag: int) -> None:
    ...
    """
    ingest_network_interface_instance_relations = """
    MATCH (aa:AWSAccount{id: $AWS_ACCOUNT_ID})-[:RESOURCE]->(i:EC2Instance)-[:NETWORK_INTERFACE]-(interface:NetworkInterface)-[:PART_OF_SUBNET]-(s:EC2Subnet)
    ...
    neo4j_session.run(
        ingest_network_interface_instance_relations, AWS_ACCOUNT_ID=aws_account_id, update_tag=update_tag,
    )
...
def load_network_interface_load_balancer_relations(neo4j_session: neo4j.Session, aws_account_id: str, update_tag: int) -> None:
    ...
    MATCH (aa:AWSAccount{id: $AWS_ACCOUNT_ID})-[:RESOURCE]->(i:LoadBalancer)-[:NETWORK_INTERFACE]-(interface:NetworkInterface)-[:PART_OF_SUBNET]-(s:EC2Subnet)
    MERGE (i)-[r:PART_OF_SUBNET]->(s)
    ON CREATE SET r.firstseen = timestamp()
    SET r.lastupdated = $update_tag
    """
    ...
    neo4j_session.run(
        ingest_network_interface_loadbalancer_relations, AWS_ACCOUNT_ID=aws_account_id, update_tag=update_tag,
    )
...
def load_network_interface_load_balancer_v2_relations(neo4j_session: neo4j.Session, aws_account_id: str, update_tag: int) -> None:
    ...
    MATCH (aa:AWSAccount{id: $AWS_ACCOUNT_ID})-[:RESOURCE]->(i:LoadBalancerV2)-[:NETWORK_INTERFACE]-(interface:NetworkInterface)-[:PART_OF_SUBNET]-(s:EC2Subnet)
    MERGE (i)-[r:PART_OF_SUBNET]->(s)
    ON CREATE SET r.firstseen = timestamp()
    SET r.lastupdated = $update_tag
    """
    ...
    neo4j_session.run(
        ingest_network_interface_loadbalancerv2_relations, AWS_ACCOUNT_ID=aws_account_id, update_tag=update_tag,
    )
