...
def get_repository_images_vulns(boto3_session, region, repository_name, repository_images):
    ...
    for image in repository_images:
        image_vuln_dict = {}
        image_vuln_dict['image_tag'] = image.get('imageTag', "")
        ...
        if response['imageDetails'][0].get('imageScanStatus', {}).get('status', None) == "COMPLETE":
            ...
            image_vuln_dict['findings'] = response.get('imageScanFindings', {}).get('findings', [])
            image_vuln_dict['scan_completed_at'] = response.get('imageScanFindings', {}).get('imageScanCompletedAt', "")
            image_vuln_dict['findings_count'] = response.get('imageScanFindings', {}).get('findingSeverityCounts', "")
            image_vulns_list.append(image_vuln_dict)
    return image_vulns_list
...
def sync(neo4j_session, boto3_session, regions, current_aws_account_id, aws_update_tag, common_job_parameters):
    for region in regions:
        ...
        for repo in repository_data:
            ...
            image_vulns[repo['repositoryUri']] = get_repository_images_vulns(
                client, region, repo['repositoryName'], image_data[repo['repositoryUri']])
        ...
        load_ecr_image_vulns(neo4j_session, image_vulns, region, current_aws_account_id, aws_update_tag)
